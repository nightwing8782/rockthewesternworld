...
import { Entry, EDITORIAL_DESKS, DeskType } from '@/types/database';
...
export default async function HomePage() {
  ...
  const leadFeature = allEntries[0];

  const commonwealthEntries = allEntries.filter(
    (e) =>
      e.metadata?.desk === 'commonwealth' ||
      e.metadata?.category === 'Dan Reads the News' ||
      e.metadata?.category === 'Wading into the Potomac'
  );
  const galleryEntries = allEntries.filter(
    (e) =>
  ...
  const logbookEntries = allEntries.filter(
    (e) =>
        ...
        {leadFeature ? (
              ...
              {allEntries.length > 1 && (
                <div className="border-t-2 border-[#1C1917] pt-4">
                  ...
                  <div className="space-y-3.5 font-serif">
                    {allEntries.slice(1, 5).map((item, idx) => (
                      <Link
          ...
          <div className="py-16 text-center border-b border-[#DDD5C7]">
            ...
            <p className="text-sm font-serif text-[#44403C] max-w-md mx-auto mb-6">
              Use the Private Studio to draft your first broadside or import your WordPress XML archive.
            </p>
            <Link
              href="/journal"
